<tr class="@if(!$day->isWeekday() or is_holiday($day)) bg-gradient-radial-info @endif" id="{{$day->format('Y-m-d')}}">
    ...
    <td>
        {{convertTime($timesheet_days->filterDay($day)->sum('duration'))}} h @if($day->isWeekday() and !is_holiday($day)) / {{convertTime($employe->employments_date($day)->sum('percent')*40/100/5)}} h @endif
    </td>
    @can('edit employe')
        <td>
    ...
    <td class="@if($timesheet_days->filterDay($day)->sum('duration')-percent_to_minutes($employe->employments_date($day)->sum('percent'))/5 > 0) text-success @else text-danger @endif">
