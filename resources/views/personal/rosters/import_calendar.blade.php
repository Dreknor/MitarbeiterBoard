...
<div class="container-fluid">
    <div class="card mb-2">
        ...
        <div class="card-body border-bottom pb-2">
            <form method="get" action="{{ route('roster.importCalendar.preview', $roster->id) }}" class="d-flex align-items-center flex-wrap" style="gap:.5rem;">
                <label class="mb-0 font-weight-bold small">Kalender:</label>
                <select name="kalender_id" class="form-control form-control-sm" style="max-width:300px;" onchange="this.form.submit()">
                    @forelse($kalender as $kal)
                        <option value="{{ $kal->id }}" @if($kal->id == $selectedKalenderId) selected @endif>
                            {{ $kal->name }}
                        </option>
                    @empty
                ...
                <form method="post" action="{{ route('roster.importCalendar.store', $roster->id) }}" id="importForm">
                    ...
                    <div class="p-3 border-top d-flex justify-content-between align-items-center flex-wrap gap-2">
                        ...
                        <button type="submit" class="btn btn-success btn-sm"
                                onclick="var c=document.querySelectorAll('.row-select:checked').length; if(c===0){alert('Bitte mindestens einen Termin auswählen.'); return false;}">
...
<script>
    document.getElementById('checkAll').addEventListener('change', function () {
        document.querySelectorAll('.row-select:not([disabled])').forEach(function(cb){ cb.checked = this.checked; }, this);
    });
</script>
@endpush
