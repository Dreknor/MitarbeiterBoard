<div class="card border-left" style="max-width: 10%;">
    ...
    <div @class(['card-body','p-0','m-0'])  style="height: 520px;">
        <ul class="time m-0 p-0" style="list-style:none;">
            @for($time=$day->copy()->setHour(8)->setMinute(0); $time->format('H:i') < '14:30'; $time->addMinutes(15))
                <li style="height:20px;line-height:20px;font-size:10px;padding-left:4px;">{{ $time->minute === 0 ? $time->format('H:i') : ''}}</li>
            @endfor
        </ul>
    </div>
