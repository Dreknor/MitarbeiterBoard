...
class AutoRosterPlanner
{
    ...
    public function suggest(Roster $roster, array $simulateAbsentIds = [], array $simulateAbsentPerDay = []): array
    {
        ...
        foreach ($events as $ev) {
            ...
            $wtStart=$best['newStart'] ?? $best['wt']->start->format('H:i'); $wtEnd=$best['newEnd'] ?? $best['wt']->end->format('H:i'); $dur=Carbon::createFromFormat('H:i',$wtStart)->diffInMinutes(Carbon::createFromFormat('H:i',$wtEnd)); if($dur>360 && !$hasBreak($best['user']->id,$dayKey)){ $mid=Carbon::createFromFormat('H:i',$wtStart)->addMinutes((int)($dur/2)-15); $breakSuggestion=['employe_id'=>$best['user']->id,'date'=>$dayKey,'start'=>$mid->format('H:i'),'end'=>$mid->copy()->addMinutes(30)->format('H:i'),'event'=>'Pause']; $totalNewBreaks++; }
