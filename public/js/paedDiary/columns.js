...
function initializeColumnsModule(dependencies) {
    ...
    function populateColumnCategoryControls() {
        ...
        if (!sel) {
            ...
            sel.innerHTML = '<option value="">-- Spaltengruppe (neu oder wählen) --</option>';
        ...
        if (!newInput) {
            ...
            newInput.placeholder = 'Neue Spaltengruppe (optional)';
    ...
    if (columnsList) {
        ...
        columnsList.addEventListener('change', e => {
            ...
            fetch(`paed-diary/column/${id}/category`, {
                ...
                if (j.success) {
                    setColumnsFeedback('Spaltengruppe aktualisiert', 'success');
                    loadAllColumns();
                } else {
